...
let main argv =
    ...
    let configureService conf =
        ...
        conf |> addCommandLineDefinition "twitterStreamUrl" (fun url -> args.TwitterStreamUrl <- Some url)
        conf |> addCommandLineDefinition "twitterSearchUrl" (fun url -> args.TwitterSearchUrl <- Some url)
